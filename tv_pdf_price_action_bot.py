...
def get_pair_currencies(pair):
    if "/" in pair:
        return set(pair.split("/"))
    return {pair[:3], pair[3:]} if len(pair) == 6 else set()
...
def touches_level(row, level, tol=0.0):
    return (float(row["low"]) - tol) <= level <= (float(row["high"]) + tol)


def rejects_support(row, support, tol=0.0):
    p = candle_parts(row)
    return float(row["low"]) <= (support + tol) and p["close"] > support


def rejects_resistance(row, resistance, tol=0.0):
    p = candle_parts(row)
    return float(row["high"]) >= (resistance - tol) and p["close"] < resistance
...
def evaluate_pattern_setup(df, i, pair="EUR/USD"):
    """
    Evaluates a candle at index i and its subsequent retracement candle(s)
    against the PDF Price Action rules (Ishaq's Binary Academy):
    1. Core Pattern: Bullish/Bearish Engulfing, Piercing Line, Dark Cloud Cover.
    2. Candidate Key Levels: Neckline (breakout level), Pattern Base/High, and Dynamic S/R.
    3. Rejection & Anatomy: Retracement must touch/test the key level (with pip tolerance)
       and close rejecting it, forming a weak counter-candle.
    4. Two-Candle Confirmation: If 1st retracement closed beyond level, checks 2nd candle rejection.
    """
    n = len(df)
    if i < 1 or i + 1 >= n:
        return None

    tol = 0.015 if "JPY" in pair else 0.00015
    p_prev = df.iloc[i - 1]
    p_curr = df.iloc[i]
    p_time = str(p_curr["time"])
    p_ts = int(pd.to_datetime(p_curr["time"]).timestamp())
    ret = df.iloc[i + 1]
    ret_time = str(ret["time"])
    ret_ts = int(pd.to_datetime(ret["time"]).timestamp())

    # 1. Bullish Engulfing
    if bullish_engulfing(df, i):
        supp_20, _ = key_levels(df, i) if i >= 20 else (float(p_curr["low"]), float(p_curr["high"]))
        neckline = float(p_prev["open"])
        base_supp = min(float(p_prev["low"]), float(p_curr["low"]))
        candidate_levels = [
            (neckline, "neckline support"),
            (base_supp, "pattern base support"),
            (supp_20, "dynamic 20-bar support"),
        ]

        for lvl, name in candidate_levels:
            if rejects_support(ret, lvl, tol=tol):
                if weak_bearish(ret):
                    return {
                        "direction": "CALL",
                        ...
                        "reason": f"Retracement touched {name}, rejected and formed weak bearish candle.",
                    }
            if float(ret["close"]) < lvl and i + 2 < n:
                second = df.iloc[i + 2]
                if rejects_support(second, lvl, tol=tol):
                    return {
                        "direction": "CALL",
                        "pattern": "Bullish Engulfing",
                        "pattern_index": i,
                        ...
                        "level": lvl,
                        "status": "CONFIRMED_SIGNAL",
                        "reason": f"Second retracement touched {name} and rejected.",
                    }
        return {
            "direction": "CALL",
            "pattern": "Bullish Engulfing",
            ...
            "level": neckline,
            "status": "PATTERN_ONLY",
            "reason": "Bullish Engulfing formed, but retracement candle did not meet weak bearish + rejection.",
        }

    # 2. Bearish Engulfing
    if bearish_engulfing(df, i):
        _, res_20 = key_levels(df, i) if i >= 20 else (float(p_curr["low"]), float(p_curr["high"]))
        neckline = float(p_prev["open"])
        base_res = max(float(p_prev["high"]), float(p_curr["high"]))
        candidate_levels = [
            (neckline, "neckline resistance"),
            (base_res, "pattern high resistance"),
            (res_20, "dynamic 20-bar resistance"),
        ]

        for lvl, name in candidate_levels:
            if rejects_resistance(ret, lvl, tol=tol):
                if weak_bullish(ret):
                    return {
                        "direction": "PUT",
                        ...
                        "level": lvl,
                        "status": "CONFIRMED_SIGNAL",
                        "reason": f"Retracement touched {name}, rejected and formed weak bullish candle.",
                    }
            if float(ret["close"]) > lvl and i + 2 < n:
                second = df.iloc[i + 2]
                if rejects_resistance(second, lvl, tol=tol):
                    return {
                        "direction": "PUT",
                        "pattern": "Bearish Engulfing",
                        "pattern_index": i,
        ...
        return {
            ...
            "level": neckline,
            "status": "PATTERN_ONLY",
            "reason": "Bearish Engulfing formed, but retracement candle did not meet weak bullish + rejection.",
        }

    # 3. Piercing Line
    if piercing_line(df, i) and prior_downtrend(df, i):
        pattern_50 = (float(p_prev["open"]) + float(p_prev["close"])) / 2.0
        supp_20, _ = key_levels(df, i) if i >= 20 else (float(p_curr["low"]), float(p_curr["high"]))
        base_supp = min(float(p_prev["low"]), float(p_curr["low"]))
        candidate_levels = [
            (pattern_50, "50% pattern level"),
            (base_supp, "pattern base support"),
            (supp_20, "dynamic 20-bar support"),
        ]
        ...
        for lvl, name in candidate_levels:
            if rejects_support(ret, lvl, tol=tol):
                return {
                    ...
                    "level": lvl,
                    "status": "CONFIRMED_SIGNAL",
                    "reason": f"Retracement tested {name} and rejected.",
                }
            if float(ret["close"]) < lvl and i + 2 < n:
                second = df.iloc[i + 2]
                if rejects_support(second, lvl, tol=tol):
                    return {
                        ...
                        "level": lvl,
                        "status": "CONFIRMED_SIGNAL",
                        "reason": f"Second candle touched {name} and rejected.",
                    }
        return {
            "direction": "CALL",
            "pattern": "Piercing Line",
            ...
            "level": pattern_50,
            "status": "PATTERN_ONLY",
            "reason": "Piercing Line formed, but retracement did not reject key level.",
        }
    ...
    if dark_cloud_cover(df, i) and prior_uptrend(df, i):
        ...
        for lvl, name in candidate_levels:
            if rejects_resistance(ret, lvl, tol=tol):
                return {
                    ...
                    "level": lvl,
                    "status": "CONFIRMED_SIGNAL",
                    "reason": f"Retracement touched {name} and rejected.",
                }
            if weak_bullish(ret):
                return {
                    "direction": "PUT",
        ...
        return {
            ...
            "level": pattern_50,
            "status": "PATTERN_ONLY",
            "reason": "Dark Cloud Cover formed, but retracement did not meet confirmation.",
        }

    return None


def find_recent_setup(df, pair="EUR/USD"):
    """
    Evaluates completed candles for active PDF trading setups:
    Pattern + Retracement candle + Key Level rejection.
    Iterates in reverse order (newest to oldest) and evaluates active
    setups whose entry candle is valid right now (entry_index == n or n - 1).
    This ensures that fresh signals are evaluated first and never shadowed
    by stale/expired setups from earlier minutes.
    """
    n = len(df)
    # Scan from newest to oldest within lookback window
    for i in range(n - 2, max(0, n - 8), -1):
        s = evaluate_pattern_setup(df, i, pair=pair)
        if s and s.get("status") == "CONFIRMED_SIGNAL":
            entry_idx = s.get("entry_index")
            if entry_idx in (n, n - 1):
                return s
    return None


def find_all_setups(df, pair="EUR/USD"):
    """
...
def relevant_news_for_pair(pair):
    currencies = get_pair_currencies(pair)
...
def log_tv_data_to_file(pair_or_content, df=None, filename=None):
    ...
    with open(filename, "a", encoding="utf-8") as f:
        f.write(entry)
    # (Uncomment below if verbose file logging output is desired)
...
def update_market_cache_and_analyze(pair, df):
    ...
    all_setups = find_all_setups(df, pair=pair)
    setup_records = []
    for s in all_setups:
    ...
    if setup_records:
        with sqlite3.connect(CACHE_DB_PATH) as conn:
            conn.executemany("""
                INSERT OR REPLACE INTO detected_setups
...
def check_forming_core_pattern(df, pair="EUR/USD"):
    """
    Checks if a core pattern (Bullish/Bearish Engulfing, Piercing Line, Dark Cloud Cover)
    formed on the very last completed candle (index len(df) - 1).
    This setup is currently awaiting its retracement confirmation candle.
    """
    n = len(df)
    if n < 5:
        return None
    ...
    if bullish_engulfing(df, i):
        ...
        return {
            ...
            "reason": "Bullish Engulfing formed; awaiting retracement candle & key level rejection."
        }
    if bearish_engulfing(df, i):
        neckline = float(p_prev["open"])
        return {
            ...
            "level": neckline,
            "candle_time": df.iloc[i]["time"],
            "reason": "Bearish Engulfing formed; awaiting retracement candle & key level rejection."
        }
    if piercing_line(df, i) and prior_downtrend(df, i):
        pattern_50 = (float(p_prev["open"]) + float(p_prev["close"])) / 2.0
        return {
            ...
            "level": pattern_50,
            "candle_time": df.iloc[i]["time"],
            "reason": "Piercing Line formed; awaiting retracement candle & key level rejection."
        }
    if dark_cloud_cover(df, i) and prior_uptrend(df, i):
        pattern_50 = (float(p_prev["open"]) + float(p_prev["close"])) / 2.0
        return {
            ...
            "level": pattern_50,
            "candle_time": df.iloc[i]["time"],
            "reason": "Dark Cloud Cover formed; awaiting retracement candle & key level rejection."
        }
    return None


def scan_pair(pair, auto_chart=False):
    df = get_candles(pair)
    ...
    # (Uncomment the line below or use --auto-charts flag)
    # ================================================================
    if auto_chart:
    ...
    setup = find_recent_setup(completed, pair=pair)

    if setup and validate_entry(completed, setup):
    ...
    # Check if a core pattern formed on the last completed candle, awaiting retracement
    forming = check_forming_core_pattern(completed, pair=pair)
    if forming:
        return {
            "blocked": False,
            "status": "AWAITING_RETRACEMENT",
            ...
            "candle_time": forming["candle_time"],
...
def main(target_pairs=None, auto_charts=False, build_on_pattern=True, verbose=False, auto_open=True, send_tg=False):
    check_settings()

    monitored_pairs = {p: PAIRS[p] for p in target_pairs if p in PAIRS} if target_pairs else PAIRS
    pair_count_str = f"{len(monitored_pairs)} FX PAIR{'S' if len(monitored_pairs) > 1 else ''}"
    if target_pairs:
        pair_names_str = ", ".join(monitored_pairs.keys())
    else:
        pair_names_str = "19 FX PAIRS"

    print("=" * 70)
    print("PDF PRICE ACTION LIVE SIGNAL BOT (V3 - TRADINGVIEW FEED)")
    print("FEED PROVIDER:    TRADINGVIEW (FX_IDC)")
    print(f"MONITORED PAIRS:  {pair_names_str}")
    ...
    print(f"BUILD ON PATTERN: {'ENABLED (builds & displays chart on any detected pattern)' if build_on_pattern else 'CONFIRMED ONLY'}")
    print(f"CONTINUOUS CHARTS:{'ENABLED (saving charts on every cycle)' if auto_charts else 'DISABLED'}")
    print(f"{pair_count_str} | 1-MINUTE CANDLES | PURE PRICE ACTION (NO INDICATORS)")
    print("=" * 70)
    ...
    while True:
        ...
        print(f"\n🔍 [{now_utc}] Scanning {len(monitored_pairs)} currency pair{'s' if len(monitored_pairs) > 1 else ''}...")

        signals_in_cycle = 0
        patterns_forming = 0

        for idx, pair in enumerate(monitored_pairs, 1):
            try:
                result = scan_pair(pair, auto_chart=auto_charts)

                if result is None:
                    if verbose or len(monitored_pairs) == 1:
                        print(f"   [{idx:02d}/{len(monitored_pairs):02d}] {pair}: Clear (watching next candle...)")
                ...
                if result.get("status") == "AWAITING_RETRACEMENT":
                    patterns_forming += 1
                    print(
                        f"   ⏳ [{pair}] Core Pattern: {result['pattern']} ({result['direction']}) "
                        f"@ {result['price']:.5f} (Awaiting retracement confirmation...)"
                    )
                    if build_on_pattern:
                        chart_file = recreate_tradingview_chart(
                            pair,
                            df=result.get("df"),
                            highlight_setup=result,
                            auto_open=auto_open,
                            send_tg=send_tg
                        )
                        print(f"   📊 [CHART CREATED] Verification chart generated: {chart_file}")
                    time.sleep(PAIR_REQUEST_DELAY)
                    continue

                # CONFIRMED SIGNAL READY FOR ENTRY
                signals_in_cycle += 1
                key = (
                    result["pair"],
                    result["pattern"],
                ...
                if key in SENT_KEYS_CACHE:
                    if verbose or len(monitored_pairs) == 1:
                        print(f"   ℹ️ [{pair}] Signal already alerted for candle {result['candle_time']}.")
                    time.sleep(PAIR_REQUEST_DELAY)
                    continue
                ...
                print(
                    f"\n{'='*70}\n"
                    f"🚨 CONFIRMED SIGNAL: {pair} | {result['direction']} | {result['pattern']}\n"
                    f"   Price: {result['price']:.5f} | Key Level: {result['level']:.5f}\n"
                    f"{'='*70}"
                )
                ...
                print(f"💾 Logging signal to file...")
                ...
                chart_file = recreate_signal_chart(result, auto_open=auto_open)
                ...
                print(f"   ⚠️ {pair}: API HTTP error: {e}")
                time.sleep(PAIR_REQUEST_DELAY)
        ...
        status_line = f"✅ [{now_utc}] Scan completed in {elapsed:.1f}s."
        if signals_in_cycle > 0:
            status_line += f" 🎯 {signals_in_cycle} NEW SIGNAL(S) DISPATCHED!"
        elif patterns_forming > 0:
            status_line += f" ⏳ {patterns_forming} pattern(s) awaiting retracement on next candle."
        else:
            status_line += " 0 signals (Market quiet, watching for fresh patterns)."
        print(status_line)
...
def run_test_signal(pair="GBP/USD"):
    ...
    message = (
        ...
        f"💰 Price: {mock_result['price']:.5f}\n"
        f"📍 Key Level: {mock_result['level']:.5f}\n\n"
        f"📌 PDF Rule:\n{mock_result['reason']}\n\n"
        "📡 Provider: TRADINGVIEW (FX_IDC)\n"
        "📰 News filter: CLEAR\n"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--pair", type=str, default=None, help="Currency pair to test/monitor (e.g. EUR/USD, GBP/USD)")
    parser.add_argument("--chart", type=str, default=None, help="Recreate chart for pair (alias for --pair)")
    parser.add_argument("--live", action="store_true", help="Run live continuous scanning (default monitors all 19 pairs; combine with --pair for single-pair live testing)")
    parser.add_argument("--monitor", action="store_true", help="Alias for --live")
    parser.add_argument("--build-on-pattern", action="store_true", default=True, help="Build and open interactive HTML chart whenever any pattern is detected (default: True)")
    ...
    parser.add_argument("--auto-charts", action="store_true", help="Continuously generate local HTML charts for all pairs on every scan cycle")
    parser.add_argument("--verbose", action="store_true", help="Print line-by-line status for every pair during scan")
    args = parser.parse_args()

    target_pair = args.pair or args.chart
    if target_pair:
        target_pair = target_pair.strip().upper()
        if "/" not in target_pair and len(target_pair) == 6:
            target_pair = f"{target_pair[:3]}/{target_pair[3:]}"

    is_live = args.live or args.monitor

    if args.test_signal:
        test_pair = target_pair if target_pair else "GBP/USD"
        run_test_signal(test_pair)
    elif target_pair and not is_live:
        ...
        pairs_list = [target_pair] if target_pair else None
        main(
            target_pairs=pairs_list,
            auto_charts=args.auto_charts,
            build_on_pattern=args.build_on_pattern,
